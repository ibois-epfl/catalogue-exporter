...
#        
        ...
        Rhino.RhinoApp.RunScript("-Export \"" + doc.Path[:-len(doc.Name)] + physicalObjectId + ".json\" -Enter", True)
        f = open(doc.Path[:-len(doc.Name)] + physicalObjectId + ".json")
        irisJson = f.read()
        f.close()
